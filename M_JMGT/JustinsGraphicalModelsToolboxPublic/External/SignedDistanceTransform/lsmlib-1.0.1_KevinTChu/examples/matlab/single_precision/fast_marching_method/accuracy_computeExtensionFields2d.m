...
for i = 1:length(grid_sizes)
  ...
  err_dist = double(distance_function-dist_exact);
  err_ext_field1 = double(extension_fields{1}-ext_field1);
  err_ext_field2 = double(extension_fields{2}-ext_field2);
...
order_str = sprintf('Order (L_%s) = %1.2f', '\infty', order_first_order_Linf);
text(200,2e-2,order_str);
order_str = sprintf('Order (L_2) = %1.2f', order_first_order_L2);
text(100,4e-5,order_str);
xlabel('N');
ylabel('Error');
