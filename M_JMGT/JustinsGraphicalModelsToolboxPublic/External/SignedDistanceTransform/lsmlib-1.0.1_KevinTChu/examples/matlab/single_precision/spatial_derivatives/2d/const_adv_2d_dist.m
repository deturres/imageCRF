...
% is the signed distance from the ellipse:
%
...
v_x = single( V_x*ones(Nx_with_ghostcells,Ny_with_ghostcells) );
v_y = single( V_y*ones(Nx_with_ghostcells,Ny_with_ghostcells) );
...
% initialize u
u_init = single( (x-0.25).^2 + 0.1*(y-0.25).^2 - 0.01 );
...
% plot results
figure(1); clf;
contour(x,y,u_WENO5_2d_RK3,[0 0],'linewidth',3);
xlabel('x');
ylabel('y');
axis([-1 1 -1 1]);
