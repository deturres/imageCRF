 ...
 *              LevelSetMethodPatchStrategy that computes the single 
 *              patch numerical routines for the level set method example 
 *              problem
 */
...
const int PatchModule::s_default_num_lobes = 3;
  ...
  Pointer<Database> db)
{
  ...
  switch (d_initial_level_set) {
    ...
    case LOBES: {
      ...
      d_num_lobes = db->getIntegerWithDefault("num_lobes", 
        s_default_num_lobes);
      break;
    }
