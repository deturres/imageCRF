  ...
  const string& object_name)
{
  ...
  // initialize d_current_component to an invalid state (to force
  // computation of velocity field on first call to computVelocityField()
  d_current_component = -1;
}
  ...
  const int component)
{
  ...
  // only carry out computation if the time or component has changed
  if ( (d_current_time == time) && (d_current_component == component) ) {
    return;
  }
  ...
  for ( int ln=0 ; ln<=finest_level ; ln++ ) {
    ...
    computeVelocityFieldOnLevel(level,time,phi_handle,component);
...
void VelocityFieldModule::computeVelocityFieldOnLevel(
  ...
  const int component) 
{
  // set velocity_field selector based on the value of component
  int velocity_field_selector;
  if (component == 0) {
    velocity_field_selector = d_velocity_field_selector_0; 
  } else {
    velocity_field_selector = d_velocity_field_selector_1;
  }
  ...
  for (PatchLevelIterator<2> pi(level); pi; pi++) { // loop over patches
    ...
    switch (velocity_field_selector) {
...
void VelocityFieldModule::printClassData(ostream& os) const
{
  ...
  os << "d_velocity_field_selector_0 = " << d_velocity_field_selector_0 << endl;
  os << "d_velocity_field_selector_1 = " << d_velocity_field_selector_1 << endl;
  ...
  Pointer<Database> db)
{
  ...
  // set velocity field selectors
  d_velocity_field_selector_0 = 
    db->getIntegerWithDefault("velocity_field_0", 0);
  d_velocity_field_selector_1 = 
    db->getIntegerWithDefault("velocity_field_1", 1);
