 ...
 * to for a 3D problem to compute the volume and surface area of regions 
...
int main(int argc, char *argv[])
{
  ...
  Pointer< CellVariable<3,LSMLIB_REAL> > phi_variable =
    new CellVariable<3,LSMLIB_REAL>("phi",1);
  Pointer< CellVariable<3,LSMLIB_REAL> > grad_phi_variable =
    new CellVariable<3,LSMLIB_REAL>("grad(phi)",3);
 
  int phi_scratch_handle = var_db->registerVariableAndContext(
    phi_variable, scratch_context, IntVector<3>(3));
 int grad_phi_plus_handle = var_db->registerVariableAndContext(
    grad_phi_variable, plus_context, IntVector<3>(0));
  int grad_phi_minus_handle = var_db->registerVariableAndContext(
    grad_phi_variable, minus_context, IntVector<3>(0));
  ...
  for ( int ln=0 ; ln < num_levels; ln++ ) {
    Pointer< PatchLevel<3> > level =
      patch_hierarchy->getPatchLevel(ln);
    Pointer< RefineSchedule<3> > sched = refine_alg.createSchedule(
...
*/
  ...
  /*
   * Compute and output volume of the region bounded by the zero level set.
   */
  LSMLIB_REAL volume = 
    LevelSetMethodToolbox<3>::computeVolumeOfRegionDefinedByZeroLevelSet(
      patch_hierarchy,
      phi_handle,
  ...
  /*
   * Compute and output area of the surface defined by the zero level set.
   */
  LSMLIB_REAL area = LevelSetMethodToolbox<3>::computeVolumeOfZeroLevelSet(
    patch_hierarchy,
    phi_handle,
    grad_phi_plus_handle,
    control_volume_handle);
  pout << "Surface area = " << area << endl;
