function [x,f,exitflag,output,myfun] = minFunc(funObj,x0,options,varargin)
...
myfun = @(x) x; % dummy function

% this block handles inputs that are matrixes (or whatever)
originalsize = size(x0);
x0 = x0(:);
funObj0 = funObj;
function [f g] = fun(x)
    [f g] = funObj0(reshape(x,originalsize));
    g = g(:);
end
funObj = @(x) fun(x);
...
if sum(abs(g)) <= tolFun
    ...
    x = reshape(x,originalsize); % modified
    return;
end
...
for i = 1:maxIter
    ...
    switch method
        ...
        case LBFGS % L-BFGS
            ...
            if i == 1
                ...
                % following 2 lines added by justin
                myfun = @(x) lbfgsC(x,old_dirs,old_stps,Hdiag); 
                %save myfun myfun
            end
            g_old = g;
    ...
    if sum(abs(t*d)) <= tolX %&& i >= 10 % && i >= 10 added by justin
        exitflag=2;
    ...
    if abs(f-f_old) < tolX %&& i >= 10 % && i >= 10 added by justin
        exitflag=2;
