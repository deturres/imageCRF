...
DualDecompositionFlipper::DualDecompositionFlipper(int width, int height, int nLabels,EnergyFunction *eng):DualDecomposition(width,height,nLabels,eng)
{
}
DualDecompositionFlipper::DualDecompositionFlipper(int nPixels, int nLabels,EnergyFunction *eng):DualDecomposition(nPixels,nLabels,eng)
{
}

DualDecompositionFlipper::~DualDecompositionFlipper()
{
}

void DualDecompositionFlipper::setFlipperFunction(MRF::SmoothCostGeneralFn fnCost_flipper)
{
  m_flippersmoothFn = fnCost_flipper;
}

void DualDecompositionFlipper::setDDFlipperFunction(MRF::SmoothCostGeneralFn fnCost_dd_flipper)
{
  m_ddflippersmoothFn = fnCost_dd_flipper;
}


void DualDecompositionFlipper::setflippedinfo(std::vector<int> **flippedinfo_f)
{
  flippedinfo_ptop = flippedinfo_f;
}


void DualDecompositionFlipper::setNodeMappingStructure(
...
void DualDecompositionFlipper::getSubGraphNodeNo(const std::vector<std::vector<int> > &node_in_subgraphs_g,
...
void DualDecompositionFlipper::setSplitType(SplitType s_var)
{
  m_splittype = s_var;
}
...
void DualDecompositionFlipper::createSubgraphs(
...
{
  if (m_splittype == SUBNONSUB)
  {

  } else if (m_splittype == TREESADD)
  {
...
void DualDecompositionFlipper::markFlippedNodesFlipper(
...
void DualDecompositionFlipper::moveTreeToSubgraph(
...
void DualDecompositionFlipper::traverseGraph(
    const std::vector<std::vector<std::vector<int> > > &node_edge_list,
...
void DualDecompositionFlipper::dumpSubGraphs(const std::vector<std::vector<int> > &subgraphs_edge_list,
               const std::vector<std::vector<int> > &edge_list)
{
...
void DualDecompositionFlipper::displayNodeEdgeType(
    const std::vector<std::vector<std::vector<int> > > &node_edge_list,
    const std::vector<std::vector<int> > &edge_list,
    const std::vector<int> &edge_type)
{
  ...
  for (int i=0; i < node_edge_list.size(); i++)
  {
    ...
    {
      printf(" Node : %d Neighbor : %d, Edge : %d \n", i, node_edge_list[i][j][0],
          node_edge_list[i][j][1] );
    }
  }
...
void DualDecompositionFlipper::moveTreeEdgesToFlipper(
    const std::vector<std::vector<int> > &edge_list,
    const std::vector<int> &edge_type,
    std::vector<int> *mark_flip_nodes,
    std::vector<int> *subgraph_edge_list,
...
void DualDecompositionFlipper::createSubgraphsTreeAdd(
    const std::vector<std::vector<std::vector<int> > > &node_edge_list,
    const std::vector<std::vector<int> > &edge_list,
    std::vector<std::vector<int> > *subgraphs_edge_list,
    std::vector<std::vector<std::vector<int> > > *subgraphs_flipped_nodes
...
void DualDecompositionFlipper::getWhatSubGraphsNodeIsUsedIn(
...
void DualDecompositionFlipper::computeDataCostForSubGraphs(
    const std::vector<std::vector<int> > &subgraphs_edge_list,
    const std::vector<std::vector<int> > &edge_list,
    std::vector<std::vector<std::vector<int> > > *node_in_subgraphs,
...
void DualDecompositionFlipper::optimizeAlg(int nIterations)
{
  if (m_grid_graph)
  {
    printf(" Dual Decomposition Flipper does not handle grid graphs, use other interface \n");
    exit(1);
  } else {
    // for non-grid graphs
    ...
    for (unsigned int i=0; i < node_in_subgraphs.size(); i++)
    {
      // make sure that each node is used at least in one subgraph.
      assert(node_in_subgraphs[i].size() != 0);
    }
    ...
    int changed = 1;
    int iter_no = 1;
    int max_iter = 200; // refers to infer from subgraph not gradient descent times
...
//      } else
      ...
      float primal_energy = totalEnergy(); // this will use fnCost directly
      if (iter_no == 1)
      {
