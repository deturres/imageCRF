...
FlipperGeneral::FlipperGeneral(int width, int height, int nLabels,EnergyFunction *eng):MRFWrapper(width,height,nLabels,eng)
{
}

FlipperGeneral::FlipperGeneral(int nPixels, int nLabels,EnergyFunction *eng):MRFWrapper(nPixels,nLabels,eng)
{
}

FlipperGeneral::~FlipperGeneral()
{
}


void FlipperGeneral::setFlipperFunction(MRF::SmoothCostGeneralFn fnCost_flipper)
{
  m_flippersmoothFn = fnCost_flipper;
}


void FlipperGeneral::setflippedinfo(std::vector<int> **flippedinfo_f)
{
  flippedinfo_ptop = flippedinfo_f;
}


void FlipperGeneral::extractEdgeList(std::vector<std::vector<std::vector<int> > > *node_edge_list,
...
int FlipperGeneral::checkValidFlipperGraph(
        ...
        { // go through all neighbors
          ...
          if (checked[neighbor] == 1)
          {
            ...
            if (edge_list_type[edge] == 1 && type_nodep == 0 && type_nodec == 1) {}
              ...
              // return(0);
              violating_edges->push_back(edge);
            }
...
int FlipperGeneral::markFlippedNodes(
    const std::vector<std::vector<std::vector<int> > > &node_edge_list,
    const std::vector<int> &edge_list_type,
    const std::vector<std::vector<int> > &edge_list,
    std::vector<int> *nodes_flipped)
...
void FlipperGeneral::computeDataCostForFlipper(
    const std::vector<std::vector<std::vector<int> > > &node_edge_list,
    const std::vector<int> &edge_list_type,
    const std::vector<std::vector<int> > &edge_list,
...
void FlipperGeneral::optimizeAlg(int nIterations)
{
  ...
  if (m_grid_graph)
  {
    ...
    std::vector<int> temp_nodes_flipped(m_nPixels, 0);
    std::vector<int> violating_edges;
    checkValidFlipperGraph(node_edge_list, edge_list_type, edge_list, &temp_nodes_flipped, &violating_edges);
    std::cout << " Number of violating edges = " << violating_edges.size() << "\n";
    for (int nj=0; nj<violating_edges.size(); nj++) {
      std::cout << violating_edges[nj] <<"  ";
    }
    if (violating_edges.size() > 0)
      return;
    temp_nodes_flipped.clear();
