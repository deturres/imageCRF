...
//chet changed value from 4 to 6
#define GCLL_BLOCK_SIZE 6  
...
public: 
	...
	//chet added new function to obtain type of item data
	void addFront(ListType item, int mflag);

    inline bool isEmpty(){if (m_head == 0) return(true); else return(false);};
    inline LinkedBlockList(){m_neighborFlag = 0; m_head = 0; m_head_block_size = GCLL_BLOCK_SIZE;}; 
    ~LinkedBlockList();
...
private:
	//chet to get to know if item data is of class Neighbor or not
	int m_neighborFlag;
