...
MeanFieldMP::MeanFieldMP(int width, int height, int nLabels,EnergyFunction *eng):MRF(width,height,nLabels,eng)
{
	m_needToFreeV = 0;
	BPinitializeAlg();
}
MeanFieldMP::MeanFieldMP(int nPixels, int nLabels,EnergyFunction *eng):MRF(nPixels,nLabels,eng)
{
	m_needToFreeV = 0;
	BPinitializeAlg();
}

MeanFieldMP::~MeanFieldMP()
{ 
	delete[] m_answer;
...
void MeanFieldMP::initializeAlg()
{
}

void MeanFieldMP::BPinitializeAlg()
{
	...
	nodeArray =     new OneNodeClusterMF[m_nPixels];
	// MEMORY LEAK? where does this ever get deleted??

	OneNodeClusterMF::numStates = m_nLabels;
...
MRF::InputType MeanFieldMP::getSmoothType()
{
  return m_smoothType;
}

EnergyFunction *MeanFieldMP::getEnergyFunction()
{
  return m_e;
}

void MeanFieldMP::setExpScale(int expScale)
{
  m_exp_scale = (float)expScale;
}

int MeanFieldMP::getNLabels()
{
  return m_nLabels;
}

int MeanFieldMP::getWidth()
{
  return m_width;
}

int MeanFieldMP::getHeight()
{
  return m_height;
}
FLOATTYPE MeanFieldMP::getExpV(int i)
{
  return m_ExpData[i];
}

FLOATTYPE *MeanFieldMP::getExpV()
{
  return m_ExpData;
}


MRF::CostVal MeanFieldMP::getHorizWeight(int r, int c)
{
  int x = c;
  int y = r;
  int pix    = x+y*m_width;
...
MRF::CostVal MeanFieldMP::getVertWeight(int r, int c)
{
  int x = c;
  int y = r;
  int pix    = x+y*m_width;
...
bool MeanFieldMP::varWeights()
{
  return m_varWeights;
}

FLOATTYPE *MeanFieldMP::getScratchMatrix()
{
  return m_scratchMatrix;
}

void MeanFieldMP::clearAnswer()
{
	memset(m_answer, 0, m_nPixels*sizeof(Label));
}


void MeanFieldMP::setNeighbors(int pixel1, int pixel2, CostVal weight)
{
  assert(0);
  //Only Grid Graphs are supported
	assert(!m_grid_graph);
...
MRF::EnergyVal MeanFieldMP::smoothnessEnergy()
{
    double eng =  0;
    double weight;
    int x,y,pix;
...
MRF::EnergyVal MeanFieldMP::dataEnergy()
{
    double eng =  0;
...
void MeanFieldMP::setData(DataCostFn dcost)
{
  m_dataFn = dcost;
  int i;
  int j;
...
void MeanFieldMP::setData(CostVal* data)
{
  int i;
  int j;
  m_D = data;
...
void MeanFieldMP::setSmoothness(SmoothCostGeneralFn cost)
{
  m_smoothFn = cost;
}
void MeanFieldMP::setSmoothness(CostVal* V)
{
  m_type = FIXED_MATRIX;
  m_V = V;
}
...
void MeanFieldMP::setSmoothness(int smoothExp,CostVal smoothMax, CostVal lambda)
{
	int i, j;
	CostVal cost;
...
void MeanFieldMP::setCues(CostVal* hCue, CostVal* vCue)
{
	m_horizWeights = hCue;
	m_vertWeights  = vCue;
}
...
void MeanFieldMP::optimizeAlg_Old(int nIterations)
{
    //int x, y, i, j, n;
    //Label* l;
    //CostVal* dataPtr;

	if ( !m_grid_graph) {printf("\nMeanFieldMP is not implemented for nongrids yet!");exit(1);}

	int numRows = getHeight();
	int numCols = getWidth();
	const FLOATTYPE alpha = 1.0f; // CPAL - was 0.8 
...
void MeanFieldMP::optimizeAlg(int nIterations)
{
    //int x, y, i, j, n;
    //Label* l;
    //CostVal* dataPtr;
	...
	for (int niter=0; niter < nIterations; niter++)
	{
	  for(int r = 0; r < numRows; r++)
	  {
	    computeMessagesLeftRightMF(nodeArray, numCols, numRows, r, alpha, this);
	  }
	  for(int c = 0; c < numCols; c++)
	  {
	    computeMessagesUpDownMF(nodeArray, numCols, numRows, c, alpha, this);
	  }
	}
