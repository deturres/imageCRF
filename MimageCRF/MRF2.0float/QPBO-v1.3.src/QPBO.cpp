	  ...
	  zero_energy(0)
{
	...
	char err_note[100];
	strcpy(err_note, "Not enough memory!");
	if (!nodes[0] || !arcs[0]) { if (error_function) (*error_function)(err_note); exit(1); }
	...
	void QPBO<REAL>::reallocate_nodes(int node_num_max_new)
{
	...
	char err_note[100];
	strcpy(err_note, "Not enough memory!");
	if (!nodes[0]) { if (error_function) (*error_function)(err_note); exit(1); }
	...
	void QPBO<REAL>::reallocate_arcs(int arc_num_max_new)
{
	...
	char err_note[100];
	strcpy(err_note, "Not enough memory!");
	if (!arcs[0]) { if (error_function) (*error_function)(err_note); exit(1); }
	...
	bool QPBO<REAL>::Save(char* filename, int format)
{
	...
	if (format == 1)
	{
		...
		for (i=0; i<GetNodeNum(); i++)
		{
			...
			char strg[50];
			strcpy(strg, "1 %d %");
			strcat(strg, type_format);
			strcat(strg, "\n");
			fprintf(fp, strg, i+1, delta);
		}
		for (e=GetNextEdgeId(-1); e>=0; e=GetNextEdgeId(e))
		{
			GetTwicePairwiseTerm(e, i, j, E00, E01, E10, E11);
	    ...
	    strcat(strg, type_format);
	    strcat(strg, "\n");
			fprintf(fp, strg, i+1, j+1, E00 + E11 - E01 - E10);
		}
		fclose(fp);
		return true;
	}
