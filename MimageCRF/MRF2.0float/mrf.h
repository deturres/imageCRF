...
public:
    ...
    //chet
    // i don't declare this as pure virtual to avoid changes to other classes that inherit
    //MRF that i am not working on
    //and so we will need to define how it works
//    virtual int traverseNeighbors(int pix1) { return -1; };
    ...
    virtual EnergyFunction *getEnergyFunction()
    {
      return m_e;
    }
...
protected:
    ...
    clock_t start;
    double timeoffset; // this variable is used to store time (and restart the timer) because clock_t
    // is of type int, it tends to overflow after about 2140 seconds.
...
public:
    ...
    double getDataCostRaw(int index);
    int updateDataCostRaw(int index, double newvalue);
    MRF::CostVal *getDataCostArray()
    {
      return m_costArray;
    }
...
public:
    ...
    // Can be used 2D grids and for general graphs
    // SmoothnessCost(MRF::SmoothCostGeneralFn costFn){m_costFn = costFn;m_type = MRF::FUNCTION;};
    SmoothnessCost(MRF::SmoothCostGeneralFn costFn){m_costFn = costFn;m_type = MRF::FUNCTION;m_varWeights=false;};

	double getSmoothnessCostRaw(int p, int q, int lp, int lq);
