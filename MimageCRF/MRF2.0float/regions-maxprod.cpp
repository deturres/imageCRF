...
OneNodeCluster::~OneNodeCluster()
{
	...
	if (nextRoundChunckFlag == 1)
	{
		for(int i = 0; i < numNodesC; i++)
		{
		  // delete[] nextRoundReceivedMsgs[0];  // commented because it causes segmentation fault.
		  // we need to delete only [0] since all 4 point to the same piece of memory.
		}
	}
		       ...
		       const int numNodes, const int msgChunkSize)
{
  ...
  // chet in the destructor
  // delete[] nextRoundReceivedMsgs[0]; in the destructor should actually be deleting it.
  // but it causes a segmentation fault and so is commented.


  for(int i = 0; i < numNodes; i++)
  {

	nodeArray[i].nextRoundChunckFlag = 1;
	nodeArray[i].numNodesC = numNodes;
...
void OneNodeCluster::ComputeMsgGeneric(FLOATTYPE *msgDest, int r, int c, MaxProdBP *mrf)
{
