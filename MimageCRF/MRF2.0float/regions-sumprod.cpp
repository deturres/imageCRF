...
OneNodeClusterMarg::OneNodeClusterMarg()
{
}

int OneNodeClusterMarg::numStates;
...
void getPsiMat(OneNodeClusterMarg &/*cluster*/, FLOATTYPE *&destMatrix, 
	       int r, int c, SumProdBP *mrf, int direction, FLOATTYPE &var_weight)
{
...
void getVarWeight(OneNodeClusterMarg &/*cluster*/,  int r, int c, SumProdBP *mrf, int direction, FLOATTYPE &var_weight)
{
  int weight_mod = 1;
  if(mrf->varWeights())
  {
...
void initOneNodeMsgMem(OneNodeClusterMarg *nodeArray, FLOATTYPE *memChunk, 
		       const int numNodes, const int msgChunkSize)
{
  FLOATTYPE *currPtr = memChunk;
  OneNodeClusterMarg *currNode = nodeArray;
...
void OneNodeClusterMarg::ComputeMsgRight(FLOATTYPE *msgDest, int r, int c, SumProdBP *mrf)
{
  ...
  if(mrf->m_type==SumProdBP::L1 || mrf->m_type==SumProdBP::L2)
  {
  ...
  else if ((mrf->getSmoothType()==MRF::FUNCTION)||(mrf->getSmoothType()==MRF::ARRAY))
  {
	...
	// CPAL - rather than maxing we need to sum
	//		- however we need to exp() the elements
	...
	FLOATTYPE tmp = 	nodeLeftMsg[leftNodeInd] + 
	  ...
	  if(leftNodeInd==0) {
		...
		//  *cmessage = tmp; // CPAL - this is the max prod method
	
      }
	  *cmessage = sum_msg; // CPAL - added
      cmessage++;
    }
  }
  else {
...
void OneNodeClusterMarg::ComputeMsgLeft(FLOATTYPE *msgDest, int r, int c, SumProdBP *mrf)
{
...
void OneNodeClusterMarg::ComputeMsgUp(FLOATTYPE *msgDest, int r, int c, SumProdBP *mrf)
{
  FLOATTYPE *nodeRightMsg = receivedMsgs[RIGHT],
    *nodeDownMsg = receivedMsgs[DOWN],
...
void OneNodeClusterMarg::ComputeMsgDown(FLOATTYPE *msgDest, int r, int c, SumProdBP *mrf)
{
...
void OneNodeClusterMarg::getBelief(FLOATTYPE *beliefVec)
{
	for(int i = 0; i < numStates; i++)
	{
...
int OneNodeClusterMarg::getBeliefMaxInd()
{
...
void computeMessagesLeftRight(OneNodeClusterMarg *nodeArray, const int numCols, const int /*numRows*/, const int currRow, const FLOATTYPE alpha, SumProdBP *mrf)
{
  const int numStates = OneNodeClusterMarg::numStates;
...
void computeMessagesUpDown(OneNodeClusterMarg *nodeArray, const int numCols, const int numRows, const int currCol, const FLOATTYPE alpha, SumProdBP *mrf)
{
  const int numStates = OneNodeClusterMarg::numStates;
  const FLOATTYPE omalpha = 1.0f - alpha;
  int i;
