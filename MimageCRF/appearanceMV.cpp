...
AppearanceMV::AppearanceMV(int numC, int dim)
{
	classname = 0;
	numClusters = numC;
	dimension = dim;
...
void AppearanceMV::setClassname(char cName[])
{
	classname = new char[strlen(cName)+1];
	strcpy(classname, cName);
}

void AppearanceMV::printParameters()
{
...
void AppearanceMV::readParameters(char filename[])
{
	char line[100];
	int check = 0;
