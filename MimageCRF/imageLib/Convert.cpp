...
void CopyPixels2(CImage2& src, CImage2& dst)
{
    ScaleAndOffset2(src, dst, 1.0f, 0.0f);
}


void ScaleAndOffset2(CImage2& src, CImage2& dst, float scale, float offset)
{
    ...
    for (int y = 0; y < sShape.height; y++)
    {
        ...
        //ScaleAndOffsetLine(&src.Pixel(0, y, 0), &dst.Pixel(0, y, 0), n, scale, offset, minVal, maxVal);
				for (int x=0; x < sShape.width; x++) {

					for (int b=0; b < sShape.nBands; b++) {
		
						double tval = src.Pixel(x,y,b) * scale + offset;
						...
						unsigned short tval2 = (unsigned short) floor(tval + 0.5);

						dst.setPixel(x, y, b, tval2);
					}
				}
...
template <class T1, class T2>
extern void ScaleAndOffsetLineWithUnknown(T1* src, T2* dst, int n,
                        float scale, float offset,
                        T2 minVal, T2 maxVal, int nD)
{
    // This routine does NOT round values when converting from float to int
    const bool scaleOffset = (scale != 1.0f) || (offset != 0.0f);
    const bool clip = (minVal < maxVal);
    if (scaleOffset)
        for (int i = 0; i < n; i++)
        {
            ...
            if (remainder > 0.45 && remainder < 0.55)
              val = nD; // unknown region
            val += offset;
            if (clip)
                val = __min(__max(val, minVal), maxVal);
            dst[i] = (T2) val;
...
template <class T1, class T2>
extern void ScaleAndOffsetWithUnknown(CImageOf<T1>& src, CImageOf<T2>& dst, float scale, float offset, int nD)
{
    // Convert between images of same shape but diffent types,
    //  and optionally scale and/or offset the pixel values
    CShape sShape = src.Shape();
    ...
    {
        int n = sShape.width * sShape.nBands;
        ScaleAndOffsetLineWithUnknown(&src.Pixel(0, y, 0), &dst.Pixel(0, y, 0),
                           n, scale, offset, minVal, maxVal, nD);
    }
}
...
extern CImageOf<T> ConvertToGray(CImageOf<T> src)
{
    ...
    for (int y = 0; y < sShape.height; y++)
    {
        ...
        for (int x = 0; x < sShape.width; x++, srcP += 4, dstP++)
        {
            ...
            float Y = (float)(0.212671 * p.R + 0.715160 * p.G + 0.072169 * p.B);
...
template void ScaleAndOffset<unsigned char, unsigned char>(CImageOf<unsigned char>&, CImageOf<unsigned char>&, float, float);
template void ScaleAndOffsetWithUnknown<unsigned char, unsigned char>(CImageOf<unsigned char>&, CImageOf<unsigned char>&, float, float, int);
