...
// new functions to handle unknown regions in images
template <class T1, class T2>
void ScaleAndOffsetLineWithUnknown(T1* src, T2* dst, int n,
                        float scale, float offset,
                        T2 minVal, T2 maxVal, int nD);

template <class T1, class T2>
void ScaleAndOffsetWithUnknown(CImageOf<T1>& src, CImageOf<T2>& dst,
                    float scale, float offset, int nD);
