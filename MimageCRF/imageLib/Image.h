...
{
public:
    CImage2(void);               // default constructor
    //CImage2(CShape s, int bandSize, png_byte*);
    // uses system-supplied copy constructor, assignment operator, and destructor

		void ReAllocate(CShape s, int bandSize);
		void ReAllocate(CShape s, int bandSize, png_bytep*);
    ...
    int BandSize(void)              { return m_bandSize; }
		...
		void setBandSize(int bsize)			{ m_bandSize = bsize; }
		void setpixSize(int psize)			{ m_pixSize = psize; }
		void setrowSize(int rsize)			{ m_rowSize = rsize; }

		png_bytep* getmemStart()				{ return m_memStart; }

		unsigned short Pixel(int x, int y, int band);
		void* PixelAddress(int x, int y, int band);
		void setPixel(int x, int y, int band, unsigned short pixval);
...
private:
    ...
    png_bytep* m_memStart;       // start of addressable memory
};


inline void* CImage2::PixelAddress(int x, int y, int band)
{
...
template <class T>
class CImageOf : public imageLib::CImage
{
public:
    CImageOf(void);
    CImageOf(CShape s);
...
{
    imageLib::CImage::ReAllocate(s, typeid(T), sizeof(T), evenIfSameShape);
}
...
{
	imageLib::CImage::ReAllocate(s, typeid(T), sizeof(T), memory, deleteWhenDone, rowSize);
}
