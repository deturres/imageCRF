...
void WriteFileTGA(imageLib::CImage img, const char* filename)
{
    ...
    {
    	imageLib::CImage u8img(sh, unsigned_8);
...
void ReadFilePGM(CByteImage& img, const char* filename)
{
    ...
    if (strcmp(dot, ".pgm") == 0) {
		read_header(stream, (char *)"PGM", 'P', '5', &width, &height, &nBands, 1);
		isGray = 1;
	}
    else if (strcmp(dot, ".ppm") == 0) {
		read_header(stream, (char *)"PGM", 'P', '6', &width, &height, &nBands, 1);
		isGray = 0;
	}
    else if (strcmp(dot, ".pmf") == 0) {
		read_header(stream, (char *)"PMF", 'P', '9', &width, &height, &nBands, 1);
		isGray = 0;
        isFloat = 1;
	}
...
void ReadImage (imageLib::CImage& img, const char* filename)
{
	if (filename == NULL)
...
void WriteImage(imageLib::CImage& img, const char* filename)
{
	if (filename == NULL)
...
void ReadImageVerb(imageLib::CImage& img, const char* filename, int verbose) {
	if (verbose)
...
void WriteImageVerb(imageLib::CImage& img, const char* filename, int verbose) {
	if (verbose)
