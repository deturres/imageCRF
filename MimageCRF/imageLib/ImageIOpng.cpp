...
void read_png_file(CImage2& img, char* file_name)
{
  ...
  png_uint_32 imgchannels   = png_get_channels(png_ptr, info_ptr);
  

	CShape s;
	s.width = (unsigned int) width;
	s.height = (unsigned int) height;
...
//	fprintf(stderr," Color Type = %d \n",(int)color_type);
	...
	row_pointers = (png_bytep*) malloc(sizeof(png_bytep) * (unsigned int)height);
	for (y=0; y<(unsigned int)height; y++)
		row_pointers[y] = (png_byte*) malloc(((unsigned int)width) * imgchannels);
  ...
  // for some reason, this next line is causing segmentation faults
