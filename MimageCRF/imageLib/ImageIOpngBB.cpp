...
//#include "zlib-1.2.3/inftrees.h"
#include "../zlib-1.2.3/inflateT.h"
//#include "zlib.h"
//#include "inflate.h"
#include <vector>
...
// Added by chetan April 18 2011
void png_free_png_ptr(png_structp &png_ptr, png_infop &info_ptr)
{
	...
	if (png_ptr->big_row_buf !=0)
		free(png_ptr->big_row_buf);
	if (png_ptr->zbuf != 0)
		free(png_ptr->zbuf);

	if (((struct inflate_stateT FAR *)png_ptr->zstream.state)->window != 0)
	{
		free(((struct inflate_stateT FAR *)png_ptr->zstream.state)->window);
	}

//	struct inflate_state FAR *state = (struct inflate_state FAR *)png_ptr->zstream.state;
//	if (state->window !=0)
//		free(state->window);

	if (png_ptr->zstream.state != 0)
		free(png_ptr->zstream.state);
...
void read_png_file(CImage2& img, char* file_name)
{
	...
	// free(info_ptr); // (chetan) added 18 April 2011 - not sure if info pointer is needed in the future or not
...
//	printf(" Address, Value of row_p %p , %p\n", &row_pointers, row_pointers);
//	for (y=0; y<2; y++)
//		printf("(perrow) Address, Value of row_p %p , %p \n", &row_pointers[y], row_pointers[y]);
  ...
  // not sure if info pointer is needed in the future or not
  png_free_png_ptr(png_ptr, info_ptr);  // (chetan) April 18 2011 not sure of this
...
void ReadFilePNG(CByteImage& img, const char* filename)
{
	...
	// chetan added and commented use of png_get_IHDR (becoz 64 bit machine reads 0 in width)
	width = info_ptr->width;
	height = info_ptr->height;
	colorType = (int) info_ptr->color_type;
	bits = (int) info_ptr->bit_depth;
