...
public:
	...
	T immax() const
	{
		...
		for(int i=1;i<nElements;i++)
			Max=__maxab(Max,pData[i]);
		return Max;
	};
	T immin() const{
		T Min=pData[0];
		for(int i=1;i<nElements;i++)
			Min=__minab(Min,pData[i]);
		return Min;
	}
	template <class T1>
	...
	// bhole
  // adding new functions to save and load double or float values
  // to save the optical flow vectors
  virtual bool saveImageData(const char* filename) const;
  virtual bool loadImageData(const char* filename);
  virtual bool saveImageData(std::ofstream& myfile) const;
  virtual bool loadImageData(std::ifstream& myfile);
...
void Image<T>::moveto(Image<T1>& image,int x0,int y0,int width,int height)
{
	...
	if(height==0)
		height=imHeight;
	int NChannels=__minab(nChannels,image.nchannels());
...
bool Image<T>::loadImage(std::ifstream& myfile)
{
	...
	{
		std::cout<<"The type of the image is different from the type of the object!"<<std::endl;
		return false;
	}
#else
...
//------------------------------------------------------------------------------------------
// function of reading or writing double valued matrices
//------------------------------------------------------------------------------------------
template <class T>
bool Image<T>::saveImageData(const char *filename) const
{
  std::ofstream myfile(filename, std::ios::out | std::ios::binary);
  if(myfile.is_open())
  {
    bool foo = saveImageData(myfile);
    myfile.close();
    return foo;
  }
  else
...
template <class T>
bool Image<T>::saveImageData(std::ofstream& myfile) const
{
  char type[16];
  sprintf(type,"%s",typeid(T).name());
  myfile.write(type,16);
...
template <class T>
bool Image<T>::loadImageData(const char *filename)
{
  std::ifstream myfile(filename, std::ios::in | std::ios::binary);
  if(myfile.is_open())
  {
    bool foo = loadImageData(myfile);
    myfile.close();
    return foo;
  }
  else
...
template <class T>
bool Image<T>::loadImageData(std::ifstream& myfile)
{
  char type[16];
  myfile.read(type,16);
// #ifdef _LINUX_MAC
  char typed[16];
  sprintf(typed,"%s",typeid(double).name());
  char typef[16];
  sprintf(typef,"%s",typeid(float).name());

  if(strcasecmp(type, typed)==0)
    sprintf(type,"double");
  else if(strcasecmp(type, typef)==0)
    sprintf(type,"float");
  else
  {
    std::cout<<"The type of the image is different from the type of the object!"<<std::endl;
    std::cout<<" Only double and float currently supported. Warning: cross-compiler not-safe!"<<std::endl;
    return false;
  }
//#else
//  cout << " Not defined ";
...
void Image<T>::imfilter(Image<T1>& image,const Image<T2>& kernel) const
{
	...
	{
		std::cout<<"Error in Image<T>::imfilter(Image<T1>& image,const Image<T2>& kernel)"<<std::endl;
		exit(-1);
	}
...
{
	int hfsize = (__maxab(hfilter.width(),hfilter.height())-1)/2;
	int vfsize = (__maxab(vfilter.width(),vfilter.height())-1)/2;
		...
		switch(type){
			...
			case collapse_max:
				...
				for(int j=1;j<nChannels;j++)
					data[i] = __maxab(data[i],pData[offset+j]);
				break;
			case collapse_min:
				data[i] = pData[offset];
				for(int j = 1;j<nChannels;j++)
					data[i]=__minab(data[i],pData[offset+j]);
				break;
		}
	}
}
...
void Image<T>::MultiplywithAcross(const Image<T1> &image1)
{
	...
	{
		std::cout<<"Error in image dimensions--function Image<T>::MultiplywithAcross()!"<<std::endl;
		return;
	}
	const T1*& pData1=image1.data();
	for(int i=0;i<nPixels;i++)
...
void Image<T>::Subtract(const Image<T1> &image1, const Image<T2> &image2)
{
	if(image1.matchDimension(image2)==false)
	{
		std::cout<<"Error in image dimensions--function Image<T>::Subtract()!"<<std::endl;
		return;
	}
	if(matchDimension(image1)==false)
		allocate(image1);
...
void Image<T>::threshold()
{
	...
	for(int i = 0;i<nPixels*nChannels;i++)
		pData[i] = __minab(__maxab(pData[i],0),ImgMax);
}
...
void Image<T>::ConvertToVisualWords(Image<T1> &result, const T2 *pDictionary, int nDim, int nVocabulary)
{
	if(nChannels !=nDim)
	{
		std::cout<<"The dimension of the vocabulary must match to the nChannels of the image"<<std::endl;
		return;
	}
																		...
																		const Image<T2>& vx,const Image<T2>& vy) const
{
	...
	for(int i  = 0; i<height; i++)
		for(int j = 0;j<width;j++)
		{
			...
			int x1 = x0+1;
			int y1 = y0+1;
			x0 = __minab(__maxab(x0,0),imWidth-1);
			x1 = __minab(__maxab(x1,0),imWidth-1);
			y0 = __minab(__maxab(y0,0),imHeight-1);
			y1 = __minab(__maxab(y1,0),imHeight-1);
			...
			for(int k = 0;k<nChannels;k++)
			{
				...
				//output.pData[offset*nChannels+k] = __maxab(__minab(output.pData[offset*nChannels+k],ImgMax),0);
