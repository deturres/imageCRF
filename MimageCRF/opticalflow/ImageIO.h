		...
		case derivative:
			...
			if(!IsFloat)
				for(int i = 0;i<nElements;i++)
					Max = __maxab(Max,abs(pImagePlane[i]));
			else
				for(int i=0;i<nElements;i++)
					Max = __maxab(Max,fabs((double)pImagePlane[i]));
			Max*=2;
			break;
		case normalized:
			...
			for(int i = 0;i<nElements;i++)
			{
				Max = __maxab(Max,pImagePlane[i]);
				Min = __minab(Min,pImagePlane[i]);
			}
			break;
	}
				...
				switch(imtype){
					case standard:
						if(IsFloat)
							...
							im.data[offset2+j] = __maxab(__minab(pImagePlane[offset1+j],255),0);
						break;
					case derivative:
...
bool ImageIO::writeImage(const QString& filename, const T*& pImagePlane,int width,int height,int nchannels,ImageType type,int quality)
{
	...
	switch(type){
		...
		case derivative:
			...
			{
				if(IsFloat)
					_Max=__maxab(_Max,fabs((double)pImagePlane[i]));
				else
					_Max=__maxab(_Max,abs(pImagePlane[i]));
			}
			break;
		case normalized:
			...
			{
				_Min=__minab(_Min,pImagePlane[i]);
				_Max=__maxab(_Max,pImagePlane[i]);
			}
			break;
	}
...
unsigned char ImageIO::convertPixel(const T& value,bool IsFloat,ImageType type,T& _Max,T& _Min)
{
	switch(type){
		case standard:
			if(IsFloat)
				return __maxab(__minab(value*255,255),0);
			else
				return __maxab(__minab(value,255),0);
			break;
		case derivative:
