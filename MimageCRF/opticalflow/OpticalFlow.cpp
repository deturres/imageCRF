...
void OpticalFlow::SanityCheck(const DImage &imdx, const DImage &imdy, const DImage &imdt, double du, double dv)
{
	...
	{
		std::cout<<"The dimensions of the derivatives don't match!"<<std::endl;
		return;
	}
	...
	std::cout<<"The mean error of |dx*u+dy*v+dt| is "<<error<<std::endl;
}
...
void OpticalFlow::estLaplacianNoise(const DImage& Im1,const DImage& Im2,Vector<double>& para)
{
	...
	for(int k = 0;k<nChannels;k++)
	{
		if(total[k]==0)
		{
			std::cout<<"All the pixels are invalid in estimation Laplacian noise!!!"<<std::endl;
			std::cout<<"Something severely wrong happened!!!"<<std::endl;
			para[k] = 0.001;
		}
		else
			para[k]/=total[k];
...
void OpticalFlow::Laplacian(DImage &output, const DImage &input, const DImage& weight)
{
	...
	{
		std::cout<<"Error in image dimension matching OpticalFlow::Laplacian()!"<<std::endl;
		return;
	}
...
bool OpticalFlow::LoadOpticalFlow(std::ifstream& myfile,DImage& flow)
{
	Image<unsigned short int> foo;
...
bool OpticalFlow::SaveOpticalFlow(const DImage& flow, const char* filename)
{
	...
	for(int i =0;i<flow.npixels();i++)
	{
		foo.data()[i*2] = (__minab(__maxab(flow.data()[i*2],-200),200)+200)*160;
		foo.data()[i*2+1] = (__minab(__maxab(flow.data()[i*2+1],-200),200)+200)*160;
	}
	return foo.saveImage(filename);
}

bool OpticalFlow::SaveOpticalFlow(const DImage& flow, std::ofstream& myfile)
{
	Image<unsigned short int> foo;
	foo.allocate(flow);
	for(int i =0;i<flow.npixels();i++)
...
bool OpticalFlow::showFlow(const DImage& flow,const char* filename)
{
	if(flow.nchannels()!=1)
	{
		std::cout<<"The flow must be a single channel image!"<<std::endl;
		return false;
	}
