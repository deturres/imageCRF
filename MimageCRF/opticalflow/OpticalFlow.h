...
public:
	...
	static bool LoadOpticalFlow(std::ifstream& myfile,DImage& flow);

	static bool SaveOpticalFlow(const DImage& flow, const char* filename);

	static bool SaveOpticalFlow(const DImage& flow, std::ofstream& myfile);
	...
	static void ComputeOpticalFlow(const DImage& Im1,const DImage& Im2,DImage& flow)
	{
		...
		{
			std::cout<<"The input images for optical flow have different dimensions!"<<std::endl;
			return;
		}
